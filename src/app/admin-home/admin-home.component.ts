...
export class AdminHomeComponent implements OnInit, OnChanges, DoCheck {
  ...
  saveStatus(){
    this.ticketService.GetAdminDetails().subscribe((result)=>{
      ...
      this.ticketService.ChangeTicketStatus(this.ticketId, this.adminKeys, this.selectedStatusValue ).subscribe((result)=>{
        this.ticketService.ViewAllTickets().subscribe((result)=>{
          this.viewAllTickets = result;
        });
      })
  ...
  saveAssignee(){
    for (let key in this.getAdmin){
      if (this.getAdmin[key] === this.selectedAdmin){
        console.log(key);
        this.ticketService.SetAssignee(key, this.ticketId, this.userId).subscribe((result)=>{
